...
def prediction(Gender,Married,Dependents,Education,Self_Employed,ApplicantIncome,
    ...
    Toatal_Income = np.log(ApplicantIncome + CoapplicantIncome)
    LoanAmount = np.log(LoanAmount)

    prediction = model.predict([[Gender,Married,Dependents,Education,Self_Employed,
                                 LoanAmount,Loan_Amount_Term,Credit_History,Property_Area,Toatal_Income]])
...
def main():
    ...
    st.title("Welocome to Loan Application")
    ...
    Credit_History = st.selectbox("Credict History",("Outstanding Loan","No Outstanding Loan"))
